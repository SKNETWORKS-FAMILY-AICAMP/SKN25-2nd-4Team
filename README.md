...
# 1. 팀 소개
</b>저희 팀은 데이터 속 숨은 패턴을 찾아내는 5명의 탐험가들로 이루어져 있습니다. 숫자와 거래 패턴의 작은 변화까지 놓치지 않고 관찰하며, 머신러닝을 통해 고객 이탈을 미리 예측하고 실무에 바로 활용 가능한 인사이트를 제공하는 것이 우리의 목표입니다.

각 팀원은 데이터 전처리, 파생변수 설계, 모델링, 시각화, 그리고 시뮬레이션까지 자신만의 전문 영역을 가지고 협력하며 프로젝트를 진행합니다. 

단순히 예측 모델을 만드는 것이 아니라, “왜 고객이 평소와 다른 행동을 하는가?”를 이해하고, 그 답을 실무 전략으로 연결하는 것이 저희 팀의 진짜 모험입니다. 데이터 속 작은 변화에서 의미를 찾아내고, 이를 기반으로 의사결정을 돕는 것이 저희 팀의 특기입니다.</b>
